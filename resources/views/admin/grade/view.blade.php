    ...
    <section class="content">
      <div class="card">
            <div class="card-header">
              ...
              <a href="/add-grade" style="align:left;">Add Grade</a>
            </div>
            <!-- /.card-header -->
              ...
              <table id="example1" class="table table-bordered table-striped table-hover">
                ...
                <tbody>
                  @foreach($grades as $grade)
                     <tr>                    
                        <td>{{$grade->id}}</td>
                        <td>{{$grade->Program_code}}</td>
                        <td>{{$grade->Grade_name}} </td>
                        <td>{{$grade->Grade_strt_pct}} </td>
                        <td>{{$grade->Grade_end_pct}} </td>
                        <td>{{$grade->Grade_points}} </td>
                        <td>{{$grade->Grade_class_name}} </td>
                        
                        <td>
                        <a href='{{url("grade/edit/{$grade->id}")}}'>edit</a>  
                        <a href='{{url("grade/delete/{$grade->id}")}}'>delete</a> 
                        </td>
                     </tr>
                  @endforeach
                </tbody>
